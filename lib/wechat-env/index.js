...
async function exec() {
    ...
    if (argv.updateAppId === 'y' && json?.wxConfig?.appId) {
        try {
            const projectPrivateConfigJson = JSON.parse(fs.readFileSync(pathResolve('project.private.config.json'), 'utf-8'));
            projectPrivateConfigJson.appid = json?.wxConfig?.appId;
            fs.writeFileSync(pathResolve('project.private.config.json'), JSON.stringify(projectPrivateConfigJson, null, 2), 'utf-8');
            logGreen('project.private.config.json appid 已覆盖')
        } catch (e) {
            logGreen('project.private.config.json appid 覆盖失败')
        }
    }
}
